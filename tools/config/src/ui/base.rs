...
use crate::{
    ...
    option::ConfigOption,
    state::ConfigState,
    types::{ConfigNode, ConfigNodelike, ConfigValue},
    ui::{
        NavigationLevel,
...
impl<'a> BaseUI<'a> {
    ...
    pub fn new(root: &'a ConfigNode, src_dir: &Path) -> Result<Self> {
    ...
    fn go_back(&mut self) -> Result<()> {
        ...
        if self.nav_stack.len() > 1 {
            ...
            if let Some(prev_level) = self.nav_stack.last().cloned() {
                ...
                self.current_items = prev_level.category.children.iter().collect();
    ...
    pub fn go_next(&mut self, category: &'a ConfigCategory) {
        ...
        self.current_items = category.children.iter().collect();
    ...
    pub fn refresh_current_items(&mut self) {
        if let Some(current_level) = self.nav_stack.last().cloned() {
            self.current_items = current_level.category.children.iter().collect();
        }
    }
