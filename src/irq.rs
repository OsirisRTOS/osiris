use crate::{error::Result, sync::{self, spinlock::{RwSpinLocked}}, types::array::Vec};
...
static HANDLERS: [RwSpinLocked<Vec<Handler, 1>>; 240] = [const { RwSpinLocked::new(Vec::new()) }; 240];
...
pub unsafe fn register_irq(vector: usize, handler: IrqHandler, userdata: Option<usize>) -> Result<()> {
    if vector >= HANDLERS.len() {
        Err(kerr!(InvalidArgument, "Invalid IRQ vector {vector}"))?;
    }

    let handler = Handler { func: handler, userdata };
    ...
    sync::atomic::irq_free(|| {
        HANDLERS[vector].write_lock().push(handler)
    })
}
