    ...
    for node in new_nodes.into_iter() {
        ...
        while let Some((current_node, path)) = queue.pop_front() {
            ...
            let should_link = match node {
                ConfigNode::Category(ref cat) => should_link(&cat.parent, &current_node.key, &current_path),
                ConfigNode::Option(ref opt) => should_link(&opt.parent, &current_node.key, &current_path),
            };
...
mod tests {
    ...
    fn link_nodes_nested() {
        ...
        let nested_opt = create_option("opt", 3, Some(ConfigKey::Qualified(".cat1.cat2".to_string())));
    ...
    fn resolve_paths_simple_dependency() {
        ...
        opt2.depends_on.push((ConfigKey::Simple("opt1".to_string()), Some(ConfigValue::Boolean(true))));
    ...
    fn resolve_paths_qualified_dependency() {
        ...
        opt2.depends_on.push((ConfigKey::Qualified(".cat1.opt1".to_string()), None));
    ...
    fn resolve_paths_unresolved_simple_dependency() {
        ...
        opt.depends_on.push((ConfigKey::Simple("nonexistent".to_string()), None));
        root.children.push(ConfigNode::Option(opt));
    ...
    fn resolve_paths_unresolved_qualified_dependency() {
        ...
        let mut opt = create_option("opt1", 1, Some(ConfigKey::Pending()));
        opt.depends_on.push((ConfigKey::Qualified(".nonexistent.path".to_string()), None));
        root.children.push(ConfigNode::Option(opt));
    ...
    fn resolve_paths_multiple_dependencies() {
        ...
        opt3.depends_on.push((ConfigKey::Simple("opt1".to_string()), Some(ConfigValue::Boolean(true))));
        opt3.depends_on.push((ConfigKey::Qualified(".opt2".to_string()), Some(ConfigValue::Boolean(false))));
