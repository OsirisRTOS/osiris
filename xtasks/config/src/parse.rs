...
fn parse_config_default(key: &Key, value: &Item, typ: &mut ConfigType) -> Result<()> {
    if let Err(_) = match value {
        ...
        return Err(Error::InvalidToml(Report::from_spanned(
            ...
            Some(key),
            value,
            format!("invalid default: expected {}, found {}", typ.type_name(), value.type_name()),
        )));
    }
    ...
    match value {
            ...
            for attrib in attribs {
                if let Value::String(s) = attrib {
                    ...
                    return Err(Report::from_spanned(
                        ...
                        Some(key),
                        value,
                        format!("invalid attribute: expected string, found {}", attrib.type_name()),
                    )
                    .into());
                }
            }
        ...
        _ => {
            return Err(Report::from_spanned(
                ...
                Some(key),
                value,
                format!("invalid attributes: expected array, found {}", value.type_name()),
            )
            .into());
        }
    }
        ...
        .map(|(name, value)| {
            ...
            if let Item::Table(table) = value {
                ...
                let mut report = Report::from_spanned(
                    ...
                    Some(parent),
                    value,
                    format!("invalid config option '{}': expected table, found {}", name, value.type_name()),
                );

                if let Some(span) = key.span() {
                    report.add_annotation(asn::Level::Help, span, Some(format!("only reserved keywords (e.g. name) are part of the category definition")));
                }
    ...
    if let Item::Table(table) = value {
        ...
        *next_id += 1;
        ...
        Err(Error::InvalidToml(Report::from_spanned(
            ...
            Some(key),
            value,
            format!("invalid config option: expected table, found {}", value.type_name()),
        )))
    }
}
    ...
    fn parse_string_with_allowed_values() {
        ...
        if let ConfigNode::Option(opt) = &nodes[0] {
            if let ConfigType::String(Some(allowed), default) = &opt.typ {
                ...
                assert!(allowed.contains(&"info".to_string()) && allowed.contains(&"debug".to_string())
