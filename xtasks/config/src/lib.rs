...
use crate::{
    category::ConfigCategory, error::{Diagnostic, Error, Report}, file::File, macros::MacroEngine, state::ConfigState, types::ConfigNode
};
...
pub fn load_state<'node>(root: &'node ConfigNode, config: Option<&Path>, ignored: &Vec<String>) -> ConfigState<'node> {
    match config {
        Some(config) => match file::load_file(config) {
            Ok(File { path, content }) => {
                ...
                match content.parse::<ImDocument<String>>().map_err(Report::from) {
                    Ok(doc) => {
                        error::fail_on_error(ConfigState::deserialize_from(&doc, root, ignored), Some(&diag))
                    }
                    Err(rep) => {
                        let msg = diag.msg(&rep);
                        log::error!("{}", error::msg_to_string(msg));
        ...
        match value {
            Item::Table(src) => {
                ...
                if let Item::Table(dst) = dst {
                    ...
                    return Err(anyhow!("type mismatch when applying preset key '{}': expected table, found {}", key, dst.type_name()).into())
                }
            },
            _ => {
