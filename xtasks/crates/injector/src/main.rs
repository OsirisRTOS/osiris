...
fn extract_section(file_path: &PathBuf, section_name: &str) -> Result<Vec<u8>, String> {
    ...
    let obj_file =
        object::File::parse(&*buffer).map_err(|e| format!("failed to parse ELF file: {}", e))?;
...
fn extract_binaries(
    ...
    let manifest_dir = manifest.parent().ok_or("invalid manifest path")?;

    let target = match target_triple {
        Some(t) => t,
        None => get_target_from_cargo_config(&manifest_dir.to_path_buf())
    ...
    let package = cmd.root_package().ok_or("no root package found")?;
    ...
    for bin_target in &package.targets {
            ...
            .any(|k| matches!(k, cargo_metadata::TargetKind::Bin))
        {
            ...
            for entry in walkdir::WalkDir::new(&binary_path)
                .into_iter()
