...
use crate::{category::ConfigCategory, error::Error, option::ConfigOption, state::ConfigState};
...
pub trait ConfigNodelike {
    ...
    fn has_attribute(&self, attribute: &Attribute) -> bool;
}
...
impl ConfigType {
    pub fn type_name(&self) -> &str {
        match self {
            ConfigType::Boolean(_) => "boolean",
            ConfigType::String(_, _) => "string",
            ConfigType::Integer(_, _) => "integer",
            ConfigType::Float(_, _) => "float",
        }
    }
}
...
pub enum Attribute {
    None,
    Deprecated,
    /// The option/category (as well as all children) are not accessible in the UI.
    /// This is purely visual and has no effect on the config structure itself.
    Hidden,
    /// The option/category is shown when its dependencies are satiesfied, if not it is completely invisible. (Not only greyed out)
    /// This is purely visual and has no effect on the config structure itself.
    NoHiddenPreview,
    /// The option/category is not stored/loaded from the config file.
    NoStore,
    /// The category is skipped in the configuration UI, e.g. all children are directly visible by the parent.
    /// This is purely visual and has no effect on the config structure itself.
    Skip,
    Experimental,
}
...
impl Display for Attribute {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Attribute::NoHiddenPreview => write!(f, "NoHiddenPreview"),
            Attribute::NoStore => write!(f, "NoStore"),
            Attribute::Skip => write!(f, "Skip"),
            Attribute::Experimental => write!(f, "Experimental"),
        }
    }
}
...
impl TryFrom<&Item> for ConfigValue {
    type Error = Error;

    fn try_from(item: &Item) -> Result<Self, Self::Error> {
        match item {
            Item::Value(Value::Boolean(b)) => Ok(ConfigValue::Boolean(*b.value())),
            Item::Value(Value::String(s)) => Ok(ConfigValue::String(s.value().to_string())),
            Item::Value(Value::Integer(i)) => Ok(ConfigValue::Integer(*i.value())),
            Item::Value(Value::Float(f)) => Ok(ConfigValue::Float(*f.value())),
            _ => Err(anyhow::anyhow!("Unsupported item type for ConfigValue").into()),
        }
    }
}
...
impl TryFrom<Value> for ConfigValue {
    ...
    fn try_from(value: Value) -> Result<Self, Self::Error> {
        match value {
            Value::Boolean(b) => Ok(ConfigValue::Boolean(b.into_value())),
            Value::String(s) => Ok(ConfigValue::String(s.into_value())),
            Value::Integer(i) => Ok(ConfigValue::Integer(i.into_value())),
            Value::Float(f) => Ok(ConfigValue::Float(f.into_value())),
            _ => Err(anyhow::anyhow!("expected a ConfigValue, found {}", value.type_name()).into()),
        }
    }
}
...
impl TryFrom<&str> for Attribute {
    ...
    fn try_from(s: &str) -> Result<Self, Self::Error> {
        match s {
            ...
            "no_hidden_preview" => Ok(Attribute::NoHiddenPreview),
            "no_store" => Ok(Attribute::NoStore),
            "skip" => Ok(Attribute::Skip),
            "experimental" => Ok(Attribute::Experimental),
            _ => Err(()),
        }
    }
}
