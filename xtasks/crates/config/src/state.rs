...
mod tests {
    ...
    fn enables_and_disables_based_on_dependency() {
        ...
        state.update_value(&1, ConfigValue::Boolean(false)).unwrap();
        assert!(!state.enabled(&2));
    }
    ...
    fn dependency_with_value_requirement() {
        ...
        let opt_level = opt(
            ...
            ConfigType::String(
                Some(vec!["debug".into(), "info".into(), "error".into()]),
                "debug".into(),
            ),
            Vec::new(),
            Vec::new(),
    ...
    fn name_to_key_conversion() {
        ...
        assert_eq!(ConfigState::<'_>::name_to_key("OTHER_VAR"), None);
    }
    ...
    fn deserialize_ignores_ignored_keys() {
        ...
        let state =
            ConfigState::deserialize_from(&doc, &root, &vec![".feature".to_string()]).unwrap();
