...
pub fn generate_uart_trampolines_h(dt: &DeviceTree) -> String {
    uart::generate_trampolines_h(dt)
}
...
mod uart {
    ...
    /// Emit a small C header that overrides the weak `__irq_<N>_handler`
    /// symbols for every DT-declared UART, dispatching into the slot
    /// position that the same registry assigns. Mirrors CAN's
    /// `can_trampolines.h`.
    pub fn generate_trampolines_h(dt: &DeviceTree) -> String {
        let buses = collect(dt);
        let mut out = String::from("#pragma once\n\n/* Generated by dtgen. */\n");
        for (i, b) in buses.iter().enumerate() {
            out.push_str(&format!(
                "void __irq_{}_handler(void) {{ uart_dispatch_by_slot({}); }}\n",
                b.irqn, i
            ));
        }
        out
