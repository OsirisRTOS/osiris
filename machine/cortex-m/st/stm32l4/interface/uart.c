...
/* RX threshold must be 1/8: the HAL's ReceiveToIdle IDLE handler reports
 * only RxXferSize-RxXferCount and never drains the RXFIFO, so any bytes
 * left below a larger threshold are stranded in hardware and corrupt the
 * next frame. At 1/8 the FIFO ISR drains every byte (RXFT re-fires while
 * RXFNE), so the FIFO is empty at IDLE. TXFIFO threshold batches TX IT. */
#define UART_RX_FIFO_THRESH UART_RXFIFO_THRESHOLD_1_8
