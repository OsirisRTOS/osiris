...
pub fn init_app(boot_info: &crate::BootInfo) -> Result<(), crate::utils::KernelError> {
    ...
    let src = NonNull::new( boot_info.args.init.begin as *mut u8).ok_or(crate::utils::KernelError::InvalidAddress)?;
    ...
    unsafe { mem.copy_from_nonoverlapping(src, len); };

    let entry = unsafe { transmute::<*const u8, extern "C" fn()>(mem.as_ptr().add(boot_info.args.init.entry_offset)) };
